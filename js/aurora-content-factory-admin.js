...
(function installExhaustiveGeneratedDocumentsView(){
  ...
  async function load(force){
    ...
    busy=true;try{
      ...
      list.innerHTML='<div class="aurore-all-generated-list">'+rows.map(x=>{const m=x.metadata&&typeof x.metadata==='object'?x.metadata:{};const pdf=!!x.pdf_url;const stage=m.lualatex_stage||m.lualatex_status||'';const active=m.lualatex_status==='processing'||m.lualatex_status==='queued';const cancelled=m.lualatex_cancel_requested===true;return '<article class="aurore-all-generated-row" data-generated-document-id="'+escAll(x.id)+'"><div class="aurore-all-generated-no">#'+escAll(x.id)+'</div><div><div class="aurore-all-generated-title">'+escAll(x.title)+'</div><div class="aurore-all-generated-meta">'+escAll([x.subject,x.level,x.class_name,x.document_type].filter(Boolean).join(' · '))+'<br>Créé le '+escAll(new Date(x.created_at).toLocaleString('fr-FR'))+(stage?' · '+escAll(stage):'')+'</div><div class="aurore-all-generated-actions">'+(pdf?'<a class="admin-btn ghost" href="'+escAll(x.pdf_url)+'" target="_blank" rel="noopener">Ouvrir le PDF</a>':'')+(active&&!cancelled?'<button type="button" class="admin-btn aurore-pdf-cancel-button" data-aurore-pdf-cancel="'+escAll(x.id)+'" title="Annuler cette génération PDF">Annuler</button>':'')+'<button type="button" class="admin-btn ghost" data-cf-render="'+escAll(x.id)+'" data-has-pdf="'+(pdf?'1':'0')+'" data-theme-color="'+escAll((m.aurore_design&&m.aurore_design.theme_color)||'#C85C0D')+'">'+(pdf?'Régénérer le PDF':'Générer le PDF')+'</button></div></div><span class="aurore-all-generated-status '+statusClass(x.status)+'">'+escAll(statusLabel(x.status))+(pdf?' · PDF prêt':' · Sans PDF')+'</span></article>'}).join('')+'</div>';
