// ---------- DOCUMENTS (connecté à Supabase) ----------
  ...
  async function allerDocuments(matiere) {
    ...
    if (serieDb) filtresBase.push(`Filiere=eq.${encodeURIComponent(serieDb)}`);

    // La navigation peut renvoyer seulement la division « C » alors que
    // Supabase stocke la classe complète (« Seconde C »). On tente d'abord
    // le filtre précis, puis on retire UNIQUEMENT Classe si celui-ci ne
    // retourne rien. Le niveau + la série restent alors actifs.
    const filtresPrecises = [...filtresBase];
    if (divisionDb && divisionDb.length > 1) {
      filtresPrecises.push(`Classe=eq.${encodeURIComponent(divisionDb)}`);
    }
    filtresPrecises.push('order=id.desc');

    async function recupererDocumentsAvecRepli(filtres) {
    ...
    try {
      let brutes = await recupererDocumentsAvecRepli(filtresPrecises);

      // Si la classe/division transmise par l'arbre n'est pas au même format
      // que la colonne Classe, on conserve les documents valides du niveau
      // et de la série au lieu d'afficher une rubrique vide.
