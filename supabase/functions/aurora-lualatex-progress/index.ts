...
Deno.serve(async req=>{
 ...
 const {data:doc,error}=await db.from("aurora_generated_documents").select("id,metadata").eq("id",id).maybeSingle();
 ...
 const metadataPatch:any={lualatex_status:progress>=100?"completed":"processing",lualatex_progress:progress,lualatex_stage:stage,lualatex_updated_at:now,lualatex_started_at:metadata.lualatex_started_at||now};const externalPatch=b?.metadata_patch&&typeof b.metadata_patch==="object"?b.metadata_patch:{};
 if(progress>=100){metadataPatch.lualatex_completed_at=now;metadataPatch.lualatex_failed_at=null;metadataPatch.lualatex_last_error=null;metadataPatch.lualatex_requested=false;}else metadataPatch.lualatex_completed_at=null;
 const {error:ue}=await db.from("aurora_generated_documents").update({metadata:{...metadata,...metadataPatch,...externalPatch},updated_at:now}).eq("id",id);
 if(ue)return out({error:ue.message},500);
 return out({ok:true,document_id:id,progress,stage,started_at:metadataPatch.lualatex_started_at,completed_at:metadataPatch.lualatex_completed_at||null});
});