...
def render_table(rows):
    ...
    spec = "|" + "l" * width + "|"
    out = [
        r"\begin{tcolorbox}[enhanced,breakable,colback=white,colframe=aurorebase,arc=7pt,boxrule=.45pt,left=7pt,right=7pt,top=7pt,bottom=7pt]",
        r"\small",
        r"\centering",
    ...
    for i, row in enumerate(cells):
        ...
        if i == 0:
            out.append(r"\hline")
    out += [r"\hline", r"\end{tabular}", r"\end{tcolorbox}"]
    return "\n".join(out)
...
def render(data):
    ...
    lines = [
        ...
        r"\fancyhead[L]{\IfFileExists{../../icon-192-1.png}{\includegraphics[height=.58cm]{../../icon-192-1.png}}{\textcolor{aurorebase}{\rule{.58cm}{.58cm}}}}",
        r"\fancyhead[R]{\textcolor{aurorebase}{\small\sffamily Section Archives}}",
        ...
        r"\tcbset{auroreblock/.style={enhanced,breakable,arc=7pt,outer arc=7pt,boxrule=.55pt,left=10pt,right=10pt,top=8pt,bottom=8pt,before skip=8pt,after skip=10pt,fonttitle=\sffamily\bfseries}}",
        r"\newcommand{\AuroreLabeledBlock}[2]{%",
        r"  \begin{tcolorbox}[auroreblock,colback=aurorelight,colframe=aurorebase,title={\textcolor{auroredeep}{#1}}]%",
        r"    #2",
        r"  \end{tcolorbox}%",
        r"}",
        r"\newcommand{\AuroreExerciseBlock}[2]{%",
        r"  \begin{tcolorbox}[auroreblock,colback=white,colframe=aurorebase,title={\textcolor{auroredeep}{Exercice #1}},leftrule=2.1pt]%",
        r"    #2",
        r"  \end{tcolorbox}%",
        r"}",
        r"\newcommand{\AuroreCorrectionBlock}[2]{%",
        r"  \begin{tcolorbox}[auroreblock,colback=aurorelight,colframe=auroredeep,title={\textcolor{auroredeep}{Corrigé — Exercice #1}},leftrule=2.1pt]%",
        r"    #2",
        r"  \end{tcolorbox}%",
        r"}",
        r"\newcommand{\AuroreFormulaBlock}[1]{%",
        r"  \begin{tcolorbox}[auroreblock,colback=aurorepale,colframe=aurorebase,arc=9pt,halign=center]%",
        ...
        r"  \includegraphics[height=.82cm]{../../icon-192-1.png}%",
        r"}{%",
        r"  \tcbox[on line,boxrule=0pt,colback=aurorelight,arc=7pt,left=8pt,right=8pt,top=5pt,bottom=5pt]{\sffamily\bfseries\color{auroredeep}A}%",
        r"}",
        r"\end{flushleft}",
        r"\vspace{1.45cm}",
